...
`sl_fp_reports`, `sl_resolver_breakers` — all in the userscript manager's
GM storage on this machine only.
...
| --- | --- | --- |
| [trw.lat](https://trw.lat) | `external-service` / lootlabs API fallback | Current URL (and, for lootlabs, the WebSocket `r:` payload) |
| [bypass.tools](https://bypass.tools) | Delegation after trw.lat fails | Current URL in `?url=` (browser navigation) |
